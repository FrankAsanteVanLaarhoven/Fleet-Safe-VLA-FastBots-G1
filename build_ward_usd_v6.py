...
print("[build_ward_usd_v6] Booting pure PXR stage construction...", flush=True)
os.makedirs(os.path.dirname(TARGET_PATH), exist_ok=True)

# Create a clean USD stage
stage = Usd.Stage.CreateNew(TARGET_PATH)
if not stage:
    print(f"[build_ward_usd_v6] ERROR: Failed to create geometry matrix at {TARGET_PATH}")
    exit(1)
...
print(f"[build_ward_usd_v6] Injecting core environment asset (S3 Mount): {HOSPITAL_URL}...", flush=True)
...
print("[build_ward_usd_v6] Defining semantic constraints and zones...", flush=True)
...
print("[build_ward_usd_v6] Baking USD to disk...", flush=True)
stage.Save()

print(f"[build_ward_usd_v6] Success: Pipeline finalized at {TARGET_PATH}", flush=True)
