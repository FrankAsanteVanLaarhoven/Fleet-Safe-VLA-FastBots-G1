...
The results reported in this paper are from single training runs.
For the full journal version, we plan five-seed experiments ($n=5$) with 95\% confidence intervals for all key metrics (SVR, reward, cost), following the methodology of Agarwal et al.~\cite{agarwal2021deep}.
We note that the monotonic convergence observed across all ten models --- with SVR decreasing consistently throughout training --- provides preliminary evidence of stability.

\textbf{Fleet-level evaluation.}
Despite the ``FLEET'' in our name, the current evaluation is entirely single-agent.
Our Fleet Coordinator model (Section~\ref{sec:extended}) trains an 8-robot task allocation policy, but does not yet evaluate shared safety envelopes.
We plan multi-robot experiments where the CBF-QP filter accounts for inter-robot collision avoidance, extending the safe set to $\mathcal{C}_\text{fleet} = \{s : h_i(s) \geq 0 \;\forall\, i \in [N]\}$ where $N$ is the fleet size.
...
Future work will extend to multi-seed statistical validation, fleet-level multi-robot evaluation, Jetson Orin hardware deployment, and real-world data collection via our Semantic Auto Data Collection pipeline.
